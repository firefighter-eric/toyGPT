...
class OPTAttentionFast(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""
    ...
    def forward(
        ...
        if past_key_value is not None:
            ...
            key_states = torch.cat([past_key_value[0], key_states], dim=2)
            value_states = torch.cat([past_key_value[1], value_states], dim=2)
        past_key_value: Tuple[Tensor, Tensor] = (key_states, value_states)

        attn_output: Tensor = torch.nn.functional.scaled_dot_product_attention(
            query=query_states,
            key=key_states,
            value=value_states,
        ...
        # be partitioned across GPUs when using tensor-parallelism.
